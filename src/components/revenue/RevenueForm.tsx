...
export type RevenueFormInputs = z.infer<typeof revenueSchema>;

interface RevenueFormProps {
  initialData?: RevenueRecord;
  onSave: (data: RevenueFormInputs) => Promise<void>;
  isSaving: boolean;
  mode: "create" | "edit";
}
...
export default function RevenueForm({ initialData, onSave, isSaving, mode }: RevenueFormProps) {
  ...
  const processSubmit: SubmitHandler<RevenueFormInputs> = async (data) => {
    await onSave(data);
  };
    ...
    <Card className="w-full max-w-2xl mx-auto shadow-lg">
      ...
      <form onSubmit={handleSubmit(processSubmit)}>
        <CardContent className="space-y-6 py-4">
          ...
          <div>
            ...
            <Textarea 
              ...
              placeholder="Detailed description of the revenue"
              rows={3}
        ...
        <CardFooter className="flex justify-end space-x-2 border-t pt-6">
          <Button type="button" variant="outline" onClick={() => router.push("/revenue")} disabled={isSaving}>
            Cancel
          </Button>
          <Button type="submit" disabled={isSaving}>
            {isSaving && <LoadingSpinner size={16} className="mr-2" />}
            {mode === "create" ? "Record Revenue" : "Save Changes"}
          </Button>
        </CardFooter>
      </form>
    </Card>
